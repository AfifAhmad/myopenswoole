...
export default function (data){
    ...
    var data = $.extend({
        ...
        filterScheme : [
            ...
            {name : "tags", caption: "Tags", type : "text"},
            {name : "short_content", caption: "Ringkasan", type : "text"},
	...
	<li v-if="pagination.current_page< pagination.max_page" class="page-item"><a class="page-link" href="javascript:void(0);" v-on:click="$root.move_page(pagination.max_page)"><span class="fa fa-angle-double-right"></span></a></li>
            ...
            "table-rows" : {
                ...
                methods : {
                    ...
                    hapus : function(){
                        if(confirm("Anda yakin akan menghapus data blog dengan judul : "+this.$props.row.title)){
                            $.ajax({
                                method : "DELETE",
                                url: this.$root.$data.app_path+"blog",
                                data :  {id : this.$props.row.blog_id},
                                success : (response)=>{
                                    var js_data = JSON.parse(response);
                                    if(js_data.success){
                                        toastr["success"]("Penghapusan data blog dengan judul "+this.$props.row.title+" berhasil");
                                        LoadTable.bind(this.$root)();
                                    } else {
                                        toastr["error"]("Penghapusan data blog dengan judul "+this.$props.row.title+" gagal");
                                    }
                                }
                            });
                        }
                    ...
                    start_edit : function(){
                        $.ajax({
                            method : "GET",
                            url: this.$root.$data.app_path+"blog/item",
                            data : {id : this.$props.row.blog_id},
                            success : (response)=>{
                                var js_data = JSON.parse(response);
                                if(js_data.success){
                                    this.$root.$data.current_id = js_data.data.blog_id;
                                    this.$root.$data.title = js_data.data.title;
                                    $(this.$root.$el).find("#inputData").find('[name="title"]')[0].value = js_data.data.title;
                                    $(this.$root.$el).find("#inputData").find('[name="url"]')[0].value = js_data.data.url;
                                    $(this.$root.$el).find("#inputData").find('[name="tags"]')[0].value = js_data.data.tags;
                                    $(this.$root.$el).find('input[name=published]').val([js_data.data.published]);
                                    CKEDITOR.instances.htmlEditor1.setData(js_data.data.short_content);
                                    CKEDITOR.instances.htmlEditor2.setData(js_data.data.content);
                                }
                            }
                        });
                    }
...
<tr>
    ...
    <td class="html_content" v-html="row.short_content"></td>
...
<div id="container">
      <div class="row">
        <div class="col-lg-12">
          ...
          <div class="card">
            <div class="card-body">
              <h5 class="card-title">{{((current_id)?"Edit":"Input")}} Blog </h5>

              <!-- General Form Elements -->
              <form class="display-invalid-feedback" id="inputData" >
                <input type="hidden" v-model="current_id" name="blog_id"/>
                <div class="row mb-3">
                ...
                <div class="row mb-3">
                  ...
                  <div class="col-sm-10">
                    ...
                    <div v-if="failed_fields.url" class="invalid-feedback">
                    {{failed_fields.url}}
                  </div>
                  </div>
                </div>
                <div class="row mb-3" >
                  ...
                  <br>
                    <textarea class="form-control" id="htmlEditor1" name="short_content"></textarea>
                    <div v-if="failed_fields.short_content" class="invalid-feedback">
                    {{failed_fields.short_content}}
                  </div>
                  </div>
                </div>
                <div class="row mb-3" >
                  ...
                  <a href="/admin/modules/media/index.php?view=list" target="_blank">Medias</a>
                  <br>
                    <textarea class="form-control" id="htmlEditor2" name="content"></textarea>
                ...
                <div class="row mb-3">
                  ...
                  <div class="col-sm-10">
                    ...
                    <div v-if="failed_fields.tags" class="invalid-feedback">
                    {{failed_fields.tags}}
                  </div>
                  </div>
                </div>
...
<thead><tr>
    ...
    <th v-bind:class="'sorting'+get_order('url')" v-on:click="set_order('url')" style="width: 300px;">URL</th>
    <th v-bind:class="'sorting'+get_order('tags')" v-on:click="set_order('tags')" style="width: 300px;">Tags</th>
    <th v-bind:class="'sorting'+get_order('name')" v-on:click="set_order('name')" style="width: 300px;">Penginput</th>
    <th v-bind:class="'sorting'+get_order('post_time')" v-on:click="set_order('post_time')" style="width: 200px;">Waktu Posting</th>
    <th v-bind:class="'sorting'+get_order('short_content')" v-on:click="set_order('short_content')" style="width: 300px;">Ringkasan</th>
  ...
  <pagination v-bind:pagination="pagination"/>
        ...
        methods : {
            ...
            batal_edit : function(){
                ...
                $(this.$root.$el).find("#inputData").find('[name="url"]')[0].value = "";
                $(this.$root.$el).find("#inputData").find('[name="title"]')[0].value = "";
                $(this.$root.$el).find("#inputData").find('[name="tags"]')[0].value = "";
                this.reset();
