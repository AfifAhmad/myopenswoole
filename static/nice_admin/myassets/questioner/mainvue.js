...
export default function (data){
    ...
    var data = $.extend({
        ...
        filterScheme : [
            ...
            {name : "outer_url", caption: "URL Luar", type : "text"},
    ...
    var LoadTable = function(){
        $.ajax({
            url : data.app_path + "questioner/list",
            method : "POST",
	...
	<li v-if="pagination.current_page< pagination.max_page" class="page-item"><a class="page-link" href="javascript:void(0);" v-on:click="$root.move_page(pagination.max_page)"><span class="fa fa-angle-double-right"></span></a></li>
            ...
            "table-rows" : {
                ...
                methods : {
                    ...
                    hapus : function(){
                        if(confirm("Anda yakin akan menghapus data questioner dengan judul : "+this.$props.row.title)){
                            $.ajax({
                                method : "DELETE",
                                url: this.$root.$data.app_path+"questioner",
                                data :  {id : this.$props.row.questioner_id},
                                success : (response)=>{
                                    var js_data = JSON.parse(response);
                                    if(js_data.success){
                                        toastr["success"]("Penghapusan data questioner dengan judul "+this.$props.row.title+" berhasil");
                                        LoadTable.bind(this.$root)();
                                    } else {
                                        toastr["error"]("Penghapusan data questioner dengan judul "+this.$props.row.title+" gagal");
                                    }
                                }
                            });
                        }
                    ...
                    start_edit : function(){
                        $.ajax({
                            method : "GET",
                            url: this.$root.$data.app_path+"questioner/item",
                            data : {id : this.$props.row.questioner_id},
                            success : (response)=>{
                                var js_data = JSON.parse(response);
                                if(js_data.success){
                                    this.$root.$data.current_id = js_data.data.questioner_id;
                                    ...
                                    $(this.$root.$el).find("#inputData").find('[name="outer_url"]')[0].value = js_data.data.outer_url;
...
<div id="container">
      <div class="row">
        <div class="col-lg-12">
          ...
          <div class="card">
            <div class="card-body">
              <h5 class="card-title">{{((current_id)?"Edit":"Input")}} Questioner </h5>

              <!-- General Form Elements -->
              <form class="display-invalid-feedback" id="inputData" >
                <input type="hidden" v-model="current_id" name="questioner_id"/>
                <div class="row mb-3">
                ...
                <div class="row mb-3">
                  <label for="inputText" class="col-sm-2 col-form-label">URL Luar</label>
                  <div class="col-sm-10">
                    <input type="text" class="form-control" name="outer_url">
                    <div v-if="failed_fields.outer_url" class="invalid-feedback">
                    {{failed_fields.outer_url}}
                  </div>
                  </div>
                </div>
                <div class="row mb-3" >
...
<thead><tr>
    ...
    <th v-bind:class="'sorting'+get_order('outer_url')" v-on:click="set_order('outer_url')" style="width: 300px;">URL Luar</th>
  ...
  <pagination v-bind:pagination="pagination"/>
        ...
        methods : {
            ...
            batal_edit : function(){
                ...
                $(this.$root.$el).find("#inputData").find('[name="outer_url"]')[0].value = "";
            ...
            submit : function(e){
                ...
                $.ajax( {
                    url: this.$data.app_path+"questioner",
                    type: 'POST',
